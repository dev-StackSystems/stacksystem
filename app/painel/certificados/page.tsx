...
export default async function CertificadosPage() {
  ...
  const where = superAdmin ? {} : { curso: { empresaId } }

  const certificados = await db.certificado.findMany({
    where,
    include: {
      aluno: { select: { nome: true, email: true } },
      curso: { select: { nome: true, empresa: { select: { nome: true } } } },
    },
              ...
              <tbody className="divide-y divide-slate-50">
                {certificados.map((cert) => (
                  <tr key={cert.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    {/* Curso */}
                    <td className="px-6 py-4">
                      <p className="font-medium text-slate-800">{cert.curso.nome}</p>
                      <p className="text-xs text-slate-400">{cert.curso.empresa.nome}</p>
                    </td>
