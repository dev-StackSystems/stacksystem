...
export default async function SegurancaPage() {
  ...
  const isEmpresaAdmin = session.user.papel === PapelUsuario.A
  if (!superAdmin && !isEmpresaAdmin) redirect("/painel")
  ...
  const logs = await db.segurancaUsuario.findMany({
    where,
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {logs.map((log) => {
                  const roleCls = ROLE_BADGE[log.usuario.papel] ?? ROLE_BADGE.F
                  const roleLabel = ROLE_LABELS[log.usuario.papel] ?? log.usuario.papel
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <p className="font-semibold text-slate-800">{log.usuario.nome}</p>
                        <p className="text-xs text-slate-400 mb-1">{log.usuario.email}</p>
