...
export default async function ConfiguracoesPage() {
  ...
  const isSystemAdmin = session.user.superAdmin
  ...
  if (!isSystemAdmin && !isEmpresaAdmin) redirect("/painel")

  // Admin de empresa (role A sem superAdmin, ou grupoIsAdmin): mostra form de configuração da própria empresa
  if (isEmpresaAdmin && !isSystemAdmin) {
    if (!session.user.empresaId) redirect("/painel")
