      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50/60">
            <th className="text-left px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Grupo</th>
          ...
          {grupos.map(grupo => (
            <tr key={grupo.id} className="hover:bg-slate-50/50 transition-colors">
              <td className="px-5 py-3.5">
                ...
                {grupo.descricao && <p className="text-xs text-slate-400 mt-0.5">{grupo.descricao}</p>}
              </td>
              {isAdmin && (
                <td className="px-5 py-3.5 text-slate-600">{grupo.empresa.nome}</td>
              )}
              <td className="px-5 py-3.5">
                {grupo.isAdmin ? (
                  <span className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full bg-amber-50 text-amber-600 border border-amber-100 font-semibold">
                    <Crown size={10} />
                    Administrador
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 font-semibold">
                    Padrão
                  </span>
                )}
              </td>
