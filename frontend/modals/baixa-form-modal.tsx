...
export interface BaixaData {
  ...
  tipo: string
  status: string
  dataPag?: Date | string | null
  dataVenc?: Date | string | null
  createdAt: Date | string
  matricula?: {
    aluno: { nome: string }
    empCurso: { nome: string }
  } | null
}
  ...
  useEffect(() => {
    if (open && baixa && mode === "edit") {
      setForm({
        ...
        tipo: baixa.tipo,
        status: baixa.status,
        dataPag: toDateInputValue(baixa.dataPag),
        dataVenc: toDateInputValue(baixa.dataVenc),
      })
    } else if (open) {
      setForm(emptyForm)
    }
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          tipo: form.tipo,
          status: form.status,
          dataPag: form.dataPag || null,
          dataVenc: form.dataVenc || null,
        }),
      })
