...
interface Props { mode: Mode; barbeiro?: BarbeiroData; trigger: React.ReactNode }
...
const EMPTY = { nome: "", apelido: "", telefone: "", cor: CORES_BARBEARIA[0] }

export function BarbeiroFormModal({ mode, barbeiro, trigger }: Props) {
  ...
  useEffect(() => {
    if (open && barbeiro && mode === "edit") {
      setForm({ nome: barbeiro.nome, apelido: barbeiro.apelido ?? "", telefone: barbeiro.telefone ?? "", cor: barbeiro.cor ?? CORES_BARBEARIA[0] })
      setAtivo(barbeiro.ativo)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const url = mode === "create" ? "/api/barbearia/equipe" : `/api/barbearia/equipe/${barbeiro!.id}`
      ...
      if (!res.ok) { const msg = data.error ?? "Erro ao salvar."; setError(msg); toast(msg, "erro"); return }
      toast(mode === "create" ? "Profissional adicionado!" : "Dados atualizados!", "sucesso")
      closeAndRefresh()
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="font-serif text-base font-bold text-slate-900 flex items-center gap-2">
                <UserCog size={16} className="text-brand-500" /> {mode === "create" ? "Novo Profissional" : "Editar Profissional"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 flex flex-col gap-3">
              <div>
                ...
                <input value={form.nome} onChange={(e) => f("nome", e.target.value)} className={inputClass} placeholder="Ex: João Silva" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelClass}>Apelido</label>
                  <input value={form.apelido} onChange={(e) => f("apelido", e.target.value)} className={inputClass} placeholder="Ex: Jão" />
                </div>
                <div>
              ...
              {mode === "edit" && (
                <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50">
                  ...
                  <span className="text-sm font-semibold text-slate-700">{ativo ? "Profissional ativo" : "Profissional inativo"}</span>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={loading} className="flex-1 bg-brand-500 hover:bg-brand-600 disabled:opacity-70 text-white font-bold py-2.5 rounded-lg text-sm transition-all flex items-center justify-center gap-2">
                  {loading && <Loader2 size={14} className="animate-spin" />}{mode === "create" ? "Adicionar" : "Salvar"}
                </button>
              </div>
            </form>
          </div>
