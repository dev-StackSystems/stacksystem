...
export interface ClienteBarbeariaData {
  id: string
  nome: string
...
interface Props { mode: Mode; cliente?: ClienteBarbeariaData; trigger: React.ReactNode }
...
const EMPTY = { nome: "", telefone: "", email: "", nascimento: "", observacao: "" }

export function ClienteBarbeariaFormModal({ mode, cliente, trigger }: Props) {
  ...
  useEffect(() => {
    if (open && cliente && mode === "edit") {
      setForm({ nome: cliente.nome, telefone: cliente.telefone ?? "", email: cliente.email ?? "", nascimento: toDate(cliente.nascimento), observacao: cliente.observacao ?? "" })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const url = mode === "create" ? "/api/barbearia/clientes" : `/api/barbearia/clientes/${cliente!.id}`
      ...
      if (!res.ok) { const msg = data.error ?? "Erro ao salvar cliente."; setError(msg); toast(msg, "erro"); return }
      toast(mode === "create" ? "Cliente cadastrado!" : "Cliente atualizado!", "sucesso")
      closeAndRefresh()
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 sticky top-0 bg-white">
              <h2 className="font-serif text-base font-bold text-slate-900 flex items-center gap-2">
                <User size={16} className="text-brand-500" /> {mode === "create" ? "Novo Cliente" : "Editar Cliente"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 flex flex-col gap-3">
              <div>
                ...
                <input value={form.nome} onChange={(e) => f("nome", e.target.value)} className={inputClass} placeholder="Nome do cliente" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" value={form.nascimento} onChange={(e) => f("nascimento", e.target.value)} className={inputClass} />
                </div>
              </div>
              <div>
                <label className={labelClass}>E-mail</label>
                <input type="email" value={form.email} onChange={(e) => f("email", e.target.value)} className={inputClass} placeholder="opcional" />
              </div>
              <div>
                <label className={labelClass}>Observação</label>
                <input value={form.observacao} onChange={(e) => f("observacao", e.target.value)} className={inputClass} placeholder="Ex: prefere máquina 2, alérgico a..." />
              </div>
              ...
              {mode === "edit" && (
                <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50">
                  ...
                  <span className="text-sm font-semibold text-slate-700">{ativo ? "Cliente ativo" : "Cliente inativo"}</span>
                </div>
              )}
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={loading} className="flex-1 bg-brand-500 hover:bg-brand-600 disabled:opacity-70 text-white font-bold py-2.5 rounded-lg text-sm transition-all flex items-center justify-center gap-2">
                  {loading && <Loader2 size={14} className="animate-spin" />}{mode === "create" ? "Cadastrar" : "Salvar"}
                </button>
              </div>
            </form>
          </div>
