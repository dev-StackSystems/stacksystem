...
interface Props { mode: Mode; categoria?: CategoriaData; trigger: React.ReactNode }
...
export function CategoriaFormModal({ mode, categoria, trigger }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const url = mode === "create" ? "/api/financeiro/categorias" : `/api/financeiro/categorias/${categoria!.id}`
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="font-serif text-base font-bold text-slate-900">{mode === "create" ? "Nova Categoria" : "Editar Categoria"}</h2>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 flex flex-col gap-3">
              <div>
                ...
                <input value={form.nome} onChange={(e) => f("nome", e.target.value)} className={inputClass} placeholder="Ex: Vendas, Aluguel, Salários..." />
              </div>
              <div className="grid grid-cols-[1fr_auto] gap-3">
                <div>
                  <label className={labelClass}>Natureza</label>
                  <select value={form.natureza} onChange={(e) => f("natureza", e.target.value)} className={selectClass}>
              ...
              {mode === "edit" && (
                <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-200 bg-slate-50">
                  ...
                  <span className="text-sm font-semibold text-slate-700">{ativo ? "Categoria ativa" : "Categoria inativa"}</span>
                </div>
              )}
