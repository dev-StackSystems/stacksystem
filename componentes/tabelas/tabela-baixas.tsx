...
import { BaixaFormModal, BaixaData, MatriculaSimples } from "@/componentes/formularios/form-baixa"
...
export function BaixaTable({ baixas, matriculas, isAdmin, canEdit }: Props) {
  ...
  const isAtrasado = (b: BaixaData) =>
    b.status === "pendente" && b.dataVencimento && new Date(b.dataVencimento) < hoje
                ...
                return (
                  <tr key={b.id} className={`hover:bg-slate-50/50 transition-colors ${atrasado ? "bg-red-50/30" : ""}`}>
                    ...
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <span className={`text-slate-500 ${atrasado ? "text-red-500 font-medium" : ""}`}>{formatDate(b.dataVencimento)}</span>
                    </td>
                    <td className="px-6 py-4 hidden lg:table-cell">
                      <span className="text-slate-500">{formatDate(b.dataPagamento)}</span>
                    </td>
                    <td className="px-6 py-4">
