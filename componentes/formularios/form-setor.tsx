...
import { MODULOS_DISPONIVEIS } from "@/tipos/sistema"
...
export function SetorFormModal({ trigger, mode = "create", setor, empresas, isAdmin, empresaId }: Props) {
  ...
  const [modulos, setModulos] = useState<string[]>(setor?.modulos.map(m => m.modulo) ?? [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const url = mode === "edit" ? `/api/setores/${setor!.id}` : "/api/setores"
  ...
  const grouped = MODULOS_DISPONIVEIS.reduce<Record<string, Array<typeof MODULOS_DISPONIVEIS[number]>>>((acc, m) => {
    ;(acc[m.grupo] ??= []).push(m)
    return acc
  }, {})
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-4 flex flex-col gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-500 mb-1">Nome do Setor *</label>
                <input
                  value={nome}
